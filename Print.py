...
def create_pdf_task5(figs,name,title,FileName,placeholder,logo1='figures/FCUP.jpg',logo2='figures/FEUP.jpg'):
	border = 'LRTB'*0
	pdf = FPDF()
	pdf.set_margins(25,18)
	pdf.add_page()
	...
	pdf.cell(45, 10,'Experimental results',border=border,align='L',ln=1)
	pdf.set_font('Arial', '' , 10)
	pdf.cell(45, 10,'Impact of the tube voltage on the ionization current',border=border,align='L',ln=1)
	pdf.cell(2, 10,'',border=0,align='L',ln=0)
	with NamedTemporaryFile(delete=False, suffix=".png") as tmpfile:
		figs[0].savefig(tmpfile.name, bbox_inches='tight')
		pdf.image(tmpfile.name,w=160,h=0)
	...
	pdf.cell(45, 10,'Impact of the tube current on the ionization current',border=border,align='L',ln=1)
